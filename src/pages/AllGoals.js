          ...
          <tbody>
            {data.goals.map((goal) => (
              <tr key={goal.goal_title}>
                <th scope="row">{goal.goal_title}</th>
                <td>{goal.goal_amount}</td>
                <td>{goal.saved_amount}</td>
                <td>{goal.goal_complete_date}</td>
                <td>{goal.is_completed ? "Complete" : "Incomplete"}</td>
              </tr>
            ))}
          </tbody>
        </Table>
        <Button variant="contained" onClick={() => navigate("/add-goal")}>
          Add Goal
        </Button>
      </ChartCard>
    </div>
